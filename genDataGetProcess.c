...
void GPSDataProcess(bool my_gpsAvail, bool my_firstTime, uint8_t* commaIndex)
{
	...
	if (my_gpsAvail){
		...
		if (my_firstTime){
			...
			g_zG[6]=(heightOld-g_zG[3])/(g_zG[0]-gpstimeOld);
		}
		heightOld=g_zG[3];
		gpstimeOld=g_zG[0];
		
		//LAT-LON
		posToGoog(&g_zG[1],xsbuff+*(commaIndex+10)+1,*(commaIndex+11)-*(commaIndex+10)-1,xsbuff[*(commaIndex+11)+1]);
		posToGoog(&g_zG[2],xsbuff+*(commaIndex+12)+1,*(commaIndex+13)-*(commaIndex+12)-1,xsbuff[*(commaIndex+13)+1]);
		g_zG[1]=g_zG[1]*xPI_180;
		...
		//VN-VE
		g_zG[4]=speed*cos(head);
		g_zG[5]=speed*sin(head);
		speed=0;
		head=0;
	}
}
...
void INSDataProcess(void)
{
	...
	g_zI[0] = ind++;
	g_zI[1] = euler[0]*0.1*xPI_180;          // euler (unit rad)
	g_zI[2] = euler[1]*0.1*xPI_180;
	g_zI[3] = euler[2]*0.1*xPI_180;
	g_zI[4] = marg[1]*0.001;           // gyro (unit rad/s)
	g_zI[5] = marg[2]*0.001;
	g_zI[6] = marg[3]*0.001;
	g_zI[7] = -marg[4]*9.80665*0.001;   // accel (unit m/s^2)
	g_zI[8] = -marg[5]*9.80665*0.001;
	g_zI[9] = -marg[6]*9.80665*0.001;
}
